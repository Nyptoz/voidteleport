...
public class VoidTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(@NonNull CommandSender sender, Command command, @NonNull String alias, String[] args) {
        ...
        switch (subCommand) {
            case "spawn", "spawninfo", "reloadspawn" -> {
            ...
            case "setspawn", "editspawn" -> {
                ...
                for (int i = flagStartIndex; i < args.length - 1; i++) {
                    ...
                    if (!activeFlag.isEmpty() && !insideQuotes) {
                        ...
                        else if (activeFlag.equals("-position") && (wordsInFlag >= 3 || arg.equalsIgnoreCase("current")))
                            activeFlag = "";
                        else if (activeFlag.equals("-rotation") && (wordsInFlag >= 2 || arg.equalsIgnoreCase("current") || arg.equalsIgnoreCase("none")))
                            activeFlag = "";
